...
struct watcherBlombergAI : public ScriptedAI
{
    watcherBlombergAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        for (std::list<Creature*>::iterator it = m_EscortList.begin(); it != m_EscortList.end(); ++it)
            (*it)->GetMotionMaster()->MovePoint(0, -10903.043945, -377.539124, 40.065773);
    ...
    void WaypointReached(uint32 i)
    {
        ...
        if (i == STITCHES_FIRST_WP)
        {
            ...
            Creature* firstWP2 = m_creature->SummonCreature(1000,
                                 -10909.53, -397.48, 41.14, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
        ...
        else if (i == STITCHES_SECOND_WP + 2)
        {
            Creature* secondWP1 = m_creature->SummonCreature(1436,
                                  -10910.273438, -519.029175, 52.996330, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
            secondWP1->SetWalk(false);

            secondWP1->GetMotionMaster()->MovePoint(0, -10904.029297, -374.387299, 39.899269);
            secondWP1->SetCombatStartPosition(-10904.029297, -374.387299, 39.899269);
            secondWP1->SetHomePosition(-10904.029297, -374.387299, 39.899269, 0.0f);
        }
        else if (i == STITCHES_THIRD_WP)
        {
            Creature* leader = m_creature->SummonCreature(1100,
                               -10618.215820, -1185.354858, 28.577244, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
            Creature* npc1 = m_creature->SummonCreature(1099,
                             -10618.901367, -1182.151367, 28.570557, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
            Creature* npc2 = m_creature->SummonCreature(1098,
                             -10616.639648, -1181.667236, 28.490921, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
            Creature* npc3 = m_creature->SummonCreature(1101,
                             -10615.996094, -1184.561646, 28.458422, 5.93, TEMPSUMMON_CORPSE_DESPAWN, 10000);
        ...
        else if (i == 61)
        {
            m_creature->SummonCreature(NPC_WATCHER_SARYS,
                                       -10574.304688, -1179.062134, 28.033930, 3.05, TEMPSUMMON_CORPSE_DESPAWN, 10000);
            m_creature->SummonCreature(NPC_WATCHER_CORWIN,
                                       -10575.126953, -1170.067261, 28.251972, 3.61, TEMPSUMMON_CORPSE_DESPAWN, 10000);
