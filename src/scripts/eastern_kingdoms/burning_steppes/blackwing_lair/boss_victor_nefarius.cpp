...
struct boss_victor_nefariusAI : public ScriptedAI
{
    ...
    {
        ACE_Stack_Trace st;
        sLog.elysium("[AI/Nefa] Victor Nefarius reset: %s", st.c_str());
        sLog.elysium("[AI/Nefa] Phases [%u:%u:%u]", phase1, phase2, phase2bis);
        sLog.elysium("[AI/Nefa] Victim: %s", m_creature->getVictim() ? m_creature->getVictim()->GetGuidStr().c_str() : "NULL");
        sLog.elysium("[AI/Nefa] Alive: %u ThreatListSize: %u", m_creature->isAlive(), m_creature->getThreatManager().getThreatList().size());
        if (Unit* victim = m_creature->getVictim())
            sLog.elysium("[AI/Nefa] Victim pos {%.2f %.2f %.2f} death=%u", victim->GetPositionX(), victim->GetPositionY(), victim->GetPositionZ(), victim->getDeathState());
        sLog.elysium("[AI/Nefa] Nefar. pos {%.2f %.2f %.2f} death=%u", m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(), m_creature->getDeathState());
