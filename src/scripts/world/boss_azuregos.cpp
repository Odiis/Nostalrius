...
SD%Complete: 90
SDComment: scripted by Ustaag < Elysium >, Rockette < Elysium >, Ivina < Elysium >.
SDCategory: Azshara
EndScriptData */
...
struct boss_azuregosAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        m_creature->RemoveAurasDueToSpell(SPELL_CURSE_OF_ELEMENTS_R1);
        m_creature->RemoveAurasDueToSpell(SPELL_CURSE_OF_ELEMENTS_R2);
        m_creature->RemoveAurasDueToSpell(SPELL_CURSE_OF_ELEMENTS_R3);
        ...
        if (m_uiManaStorm_Timer < diff)
        {
            if (Unit* target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                if (DoCastSpellIfCan(target, SPELL_MANASTORM) == CAST_OK)
                {
                    ...
                    // No arcane vacuum right after manastorm.
                    if (m_uiReflect_Timer < 4000)
                        m_uiReflect_Timer = urand(4000, 6000);
                }
        }
