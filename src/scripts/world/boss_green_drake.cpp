...
En theorie, Ysondre devrait etre full script à present :) Les loots seront ouverts quand il aura été testé et confirmé sur Elysium.
...
uint32 GetDrakeIndex(uint32 entry)
{
    switch (entry)
    {
        case NPC_YSONDRE:   return 0;
        case NPC_TAERAR:    return 1;
        case NPC_EMERISS:   return 2;
        default:            return 3;
    }
}
...
struct boss_greendrakeAI : public ScriptedAI
{
    ...
    {
        // Find entry
        static const int possibleDrakes[4] = {NPC_LETHON, NPC_EMERISS, NPC_YSONDRE, NPC_TAERAR};
        int permutation[4] = {NPC_LETHON, NPC_EMERISS, NPC_YSONDRE, NPC_TAERAR};
        uint32 randomSeed = sObjectMgr.GetSavedVariable(VARIABLE_GREEN_DRAKE_SPAWN_SEED, 0);
        for (int i = 0; i < 5; ++i)
        {
            uint32 perm1 = randomSeed % 4;
            uint32 perm2 = randomSeed % 3;
            if (perm2 >= perm1)
                ++perm2;
            std::swap(permutation[perm1], permutation[perm2]);
            randomSeed /= 4*3;
        }
        int drakeIdx = GetDrakeIndex(m_creature->GetDBTableEntry());
        m_creature->UpdateEntry(permutation[drakeIdx]);
    ...
    void JustDied(Unit* pKiller)
    {
        DespawnDreamFogs();
        DespawnMushrooms();
        int idx = GetDrakeIndex(m_creature->GetDBTableEntry()) + VARIABLE_GREEN_DRAKE_RSPW_FIRST;
        sObjectMgr.SetSavedVariable(idx, time(NULL), true);

        for (int i = VARIABLE_GREEN_DRAKE_RSPW_FIRST; i < VARIABLE_GREEN_DRAKE_RSPW_LAST; ++i)
            if (!sObjectMgr.GetSavedVariable(i, 0)) // This one is not dead
                return;
        // I am the last of the drakes to die
        for (int i = VARIABLE_GREEN_DRAKE_RSPW_FIRST; i < VARIABLE_GREEN_DRAKE_RSPW_LAST; ++i)
            sObjectMgr.SetSavedVariable(i, 0, true);

        // Respawn delay
        uint32 respawnDelay = urand(4*24*3600, 7*24*3600);
        sObjectMgr.SetSavedVariable(VARIABLE_GREEN_DRAKE_RESPAWN_TIME, time(NULL) + respawnDelay, true);
        sObjectMgr.SetSavedVariable(VARIABLE_GREEN_DRAKE_SPAWN_SEED, rand(), true);
    }
    ...
    {
        uint32 rspTime = sObjectMgr.GetSavedVariable(VARIABLE_GREEN_DRAKE_RESPAWN_TIME, time(NULL) + 7*24*3600);
        time_t now = time(NULL);
        if (rspTime != m_creature->GetRespawnTime())
        {
            m_creature->SetRespawnTime(rspTime - now);
    ...
    {
        sObjectMgr.SetSavedVariable(VARIABLE_GREEN_DRAKE_RESPAWN_TIME, time(NULL) + urand(4*24*3600, 8*24*3600));
                ...
                m_creature->GetHealthPercent() >= float(5))
        {
            switch (m_creature->GetEntry())
            {
                ...
                case NPC_EMERISS:
                    ...
                    break;
                case NPC_LETHON:
                    DoCastSpellIfCan(m_creature, SPELL_DRAWSPIRIT);
                    break;
            }
            m_ui25PcModifier++;
        }
