...
bool ChatHandler::HandleDbcExportSpellCommand(char *args)
{
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        oss << spell->Id << ",";
        if (count == 1) if (count == 1) tblstruct << "`Id` int(10) unsigned NOT NULL default '0',\n";
        oss << spell->Category << ",";
...
bool ChatHandler::HandleFlushHonorCommand(char *args)
{
    if (sObjectMgr.IsInMaintenance())
    {
        PSendSysMessage("Une maintenance est deja en cours.");
        return true;
    }
    uint32 BeginFlushHonorDate = sWorld.GetDateToday() - 7;
    sObjectMgr.DoWeeklyHonorMaintenance();
    PSendSysMessage("Maintenance de la semaine %u effectuee", BeginFlushHonorDate);
    return true;
}
...
bool ChatHandler::HandleHonorSetRPCommand(char *args)
{
    float value = 0.0f;
    sscanf(args, "%f", &value);
    m_session->GetPlayer()->SetStoredHonor(value);
    m_session->GetPlayer()->UpdateHonor();
...
bool ChatHandler::HandleDebugLoSAllowCommand(char* args)
{
    ...
    {
        sLog.elysium("[VMAPS] Collision for model '%s' %s by %s (guid %u)", spawn->name.c_str(), value ? "disabled" : "enabled", m_session->GetPlayer()->GetName(), m_session->GetPlayer()->GetGUIDLow());
        if (value)
        {
...
std::string GetCustomFlagName(customFlag flagId)
{
    switch (flagId)
    {
        ...
        case CUSTOM_FLAG_FROM_ELYSIUM:
            return "Elysium avant fusion Blackrock";
            break;
        case CUSTOM_FLAG_FROM_BLACKROCK:
            return "Blackrock";
            break;
        case CUSTOM_FLAG_FROM_ELYSIUM_2:
            return "Elysium apres fusion Blackrock";
            break;
        case CUSTOM_FLAG_FROM_PRISMATIA:
            return "Prismatia";
            break;
        case CUSTOM_FLAG_FROM_ELYSIUM_3:
            return "Apres fusion Prismatia-Elysium";
            break;
...
//#define DO_LINK(b,a)     "|Helysium:0|h"a"|h"
...
bool ChatHandler::HandleFactionChangeItemsCommand(char* c)
{
    for (uint32 id = 0; id < sItemStorage.MaxEntry; id++)
    {
        ...
        if (!canEquip)
        {
            ...
            for (uint32 id2 = 0; id2 < sItemStorage.MaxEntry; id2++)
...
bool ChatHandler::HandleRecupCommand(char* c)
{
    ...
    if (recupItems)
    {
        do
        {
            ...
            if (!item_proto)
            {
                PSendSysMessage("/!\ Item %u introuvable.", itemId);
                continue;
            }
            Item* item = Item::CreateItem(itemId, quantity, target);
            if (!item)
            {
                PSendSysMessage("/!\ Item %u x %u impossible a creer.", itemId, quantity);
                continue;
            }
            item->SaveToDB();

            MailDraft draft;
            draft.SetSubjectAndBody("Importation personnage", "Toute l'equipe de Elysium vous souhaite la bienvenue.");
            draft.AddItem(item);
...
/*
Implementation des commandes de reload.
*/

#define RELOAD_TABLE_COMMAND(tableName, func, chatCmdName) \
bool ChatHandler::chatCmdName(char* )\
{\
    func ();\
    SendSysMessage(">> Table `" tableName "` reloaded.");\
    return true;\
}
