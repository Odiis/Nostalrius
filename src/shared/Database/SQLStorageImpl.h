...
template<class T>
void SQLStorageLoaderBase<T>::Reload(SQLStorage &store)
{
    sLog.outString("Rechargement de `%s`", store.table);
    uint32 maxi;
    Field *fields;
    QueryResult *result  = WorldDatabase.PQuery("SELECT MAX(%s) FROM %s", store.entry_field, store.table);
    if (!result)
    {
        sLog.outError("Erreur : table `%s` n'existe pas ? Abandon du reload.", store.table);
        return;
    }
    ...
    {
        fields = result->Fetch();
        uint32 uiNewRecordCount = fields[0].GetUInt32();
        delete result;
        if (store.RecordCount != uiNewRecordCount)
        {
            sLog.outError("Reload de `%s` : nombre d'enregistrements different. Abandon.", store.table);
            return;
        }
    }
    else
    {
        sLog.outError("Erreur : Impossible de recuperer le nombre d'enregistrements dans `%s`. Abandon.", store.table);
        return;
    }
    ...
    if (!result)
    {
        sLog.outError("Erreur : table `%s` vide. Abandon du reload.", store.table);
        return;
    }
    ...
    if(store.iNumFields != result->GetFieldCount())
    {
        sLog.outError("`%s` : %u champs au lieu de %u. Abandon.", store.table, result->GetFieldCount(), store.iNumFields);
        delete result;
        return;
    }
    ...
    {
        fields = result->Fetch();
        //char *p=(char*)&store.data[recordsize*count];
        uint32 currIndex = fields[0].GetUInt32();
        char *p = (char*)store.LookupEntry<T>(currIndex);
        if (!p) // Anciennement pas charge ...
            continue;
